...
**Contexte :** Votre application `Yabdos` est maintenant installable (grâce au Manifeste du TP1). Mais si vous coupez le réseau, vous obtenez le dinosaure Chrome. L'objectif d'aujourd'hui est de transformer votre application en une forteresse "Offline-First" en maîtrisant le proxy réseau : le **Service Worker**.
...
if ("serviceWorker" in navigator) {
  ...
  window.addEventListener("load", async () => {
    try {
      ...
      console.log(
        "[App] Service Worker enregistré avec succès ! Scope :",
        registration.scope,
      );
      ...
      registration.addEventListener("updatefound", () => {
        console.log(
          "[App] Une nouvelle version du Service Worker est en cours d'installation...",
        );
      });
    } catch (error) {
      console.error(
        "[App] Échec de l'enregistrement du Service Worker :",
        error,
      );
    }
  });
  ...
  console.warn(
    "[App] Les Service Workers ne sont pas supportés sur ce navigateur.",
  );
}
```
        ...
        return Promise.all(
          cacheNames.map((cacheName) => {
            ...
            if (!cacheWhitelist.includes(cacheName)) {
              console.log(
                `[SW] Suppression de l'ancien cache obsolète : ${cacheName}`,
              );
...
async function loadArticles() {
  ...
  try {
    ...
    // Génération du HTML
    const html = exercices
      .map(
        (exercice) => `
            <div class="card">
                <img src="${exercice.image}" alt="${exercice.title}">
    ...
    contentDiv.innerHTML =
      '<p style="color:red">Impossible de charger les exercices.</p>';
  }
}
...
self.addEventListener("fetch", (event) => {
  ...
  if (url.pathname === "/api/exercices") {
    // On construit une réponse artificielle (Mock)
    const fakeData = [
      {
        title: "PWA : Le Futur du Web",
        image:
          "[https://dummyimage.com/600x400/000/fff&text=PWA](https://dummyimage.com/600x400/000/fff&text=PWA)",
      },
      {
        title: "Service Worker Power",
        image:
          "[https://dummyimage.com/600x400/000/fff&text=SW](https://dummyimage.com/600x400/000/fff&text=SW)",
      },
      {
        title: "Offline First",
        image:
          "[https://dummyimage.com/600x400/000/fff&text=Offline](https://dummyimage.com/600x400/000/fff&text=Offline)",
      },
    ];
  ...
  // Remplacer aléatoirement 1 image sur 5
    ...
    event.respondWith(
      fetch("[https://picsum.photos/200/300](https://picsum.photos/200/300)"),
    );
  }
...
👉 **Testez :** Actualisez la page. La liste doit s'afficher alors que l'URL `/api/exercices` n'existe pas sur votre serveur. C'est la preuve que le SW a intercepté la requête.
